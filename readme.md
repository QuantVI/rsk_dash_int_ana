...
- connection to the stock prices of he vehicles involved
...
- how much can you as a bank lose on defaulting loans, and still make money?
  - limit losses by limiting the default rate
  - charge enough of an interest rate
  - these are NOT unsecured loans. The asset it part of the recovery rate.
- FICO score
- Internal scoring method
   ...
   - monthly income
   - bank savings
- A settlement or a payment plan helps the recovery rate even after going from delinquency into default
...
- "Portfolio-At-Risk" measurement
  - e.g. PAR 30 is the percentage of outstanding loan balance that is 30 days or more overdue
...
- There will be some correlation between the stocks for the vehicle companies, and the assets involved in the other major parts BIZ-INTEL, UZR/HOSPITALITY, FIN MRKT INDICES
  ...
  - how can the default risk of YOUR firm itself be assessed?
  - if you have a default risk, then this would be applied to your CVA for other types of financing you might enter into 

<hr>

B. Dashboarding and Analytics
- Evergreen, industry-less, industry-agnostic metrics about our first set of data: the vehicle assets
- Three addressed-dimension graphs, that use size, color, and or position to add additional dimensions of comparison to a 2D plot
...
- Can we A/B test strategies in our portfolio?
  - how would you replicate the idea of "users" only being in one group or the other?
...
- Could there be a behavioral component to the people we lend to?
  - for example, on-time payment for people in certain groups lead to forbearance periods, or slightly lowered premiums?
...
- Look up causal models for sales projection/prediction

<hr>

C. Data Integration, APIs, System Analysis, Databases, Data Pipelines, User-level Profiles
- What de we have at the User level?
- Is there a User? 
- Is it our consumer? Such as who we lend to
1. Can we create a User profile?
  - what would make the User profile "rich"?
  - the original concept
    ...
    - other preferences
    - Is this just demographic data?
      - https://www.yieldify.com/blog/types-of-market-segmentation/
      - demographic, psychographic, geographic, behavioral
    - also, show when their "NEXT STAY" is (maybe like a prediction on what they will be and when
...
2. APIs
  - here's where data-feeds come in
  ...
  - Then there's Indeed, which has a true API.
    - The API endpoint exists. We would need to make our own functionality to interact with it
    - At the least, we'd need to understand the API calls and what they return to facilitate what we need
  ...
  - Glassdoor
    - another potential location for employer ratings, sentiment, and job availability, e.g. soft local economy score
  - LinkedIn
    - In addition to jobs data, this could be a chance to do the "name association" thing
  ...
  - This is a good place to make a database decision, and it combines with the Part-A "UACC" stuff.
    ...
    - ElasticSearch: How hard would t be to relate an ES-compliant database?
      ...
      - should the user profiles live here? It is JSON. Easy display, and compatibility with a web front-end
